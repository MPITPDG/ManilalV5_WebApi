        ...
        <tr class="summaryDetail">
          <xsl:choose>
            <xsl:when test="AUTHORISATIONSTATUS='U'">
              ...
              <td class="summaryDetail"  align="center" >
                <img src="../../include/img/rejected.gif" class="cur"  onclick="parent.fnReject_JV_Authorisation('{ID}','{REQCODE}','{PSTATUS}')" alt="Cancel '{PSTATUS}' {REQJVCODE}" />
              </td>
            </xsl:when>
          </xsl:choose>
          <td class="summaryDetail" NOWRAP="" align="left">
            <label runat="server" class="cur" onclick="parent.fnView_Request('{ID}','{REQCODE}','{PSTATUS}')" title="View Request {REQCODE}" >
