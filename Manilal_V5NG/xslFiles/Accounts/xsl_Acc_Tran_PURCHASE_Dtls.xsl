			...
			<xsl:for-each select="//NewDataSet/Table">
				...
				<tr class="summaryDetail">
					<td class="summaryDetail"  align="center" >
						<img src="../../include/img/edit_item.gif" class="cur"  onclick="parent.fnPopulate_PURCHASE_Dtls('{ID}','{ENTRYNO}','{CLIENT}','{JOBNO}','{DEDUCTION}','{AMOUNT}','{USERNAME}','{NARRATION}','{IS_NOT_EDIT_ABLE}','{ITEMCODE}','{VGUID}','{ACCOUNT_NAME}','{ITEM}','{ACC_PURCHASEDTLS_ID}','{CB_STATUS}','{ISJOBREPORT}','{ITEM_DATAVALUE}')" alt="Edit Details {ACCOUNT_NAME}"/>
						<img src="../../include/img/thrash.gif" class="cur"  onclick="parent.fnDelete_PURCHASE_Dtls('{ID}','{VGUID}','{IS_NOT_EDIT_ABLE}','{ACC_PURCHASEDTLS_ID}')" alt="Delete Details {ACCOUNT_NAME}" />
					</td>
