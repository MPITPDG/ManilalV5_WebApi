            ...
            <tr class="summaryDetail">
              <xsl:if test="$MPCARGO!='1'">
                <td class="summaryDetail" NOWRAP="" align="left">
                  <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
                  <xsl:if test="ISLAST!='Y'">
                    <img src="../../INCLUDE/img/thrash.gif" class="cur"  onclick="parent.fnRemove_MAWBNO('{PK_ID}','{JOB_NO}')" alt="Remove this JobNo[{JOB_NO}] from HAWB."/>
                  </xsl:if>
                  <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
                </td>
              </xsl:if>
